...
const createRequestToken = () => {
  return fetchRequestToken()
    .then(({ request_token: requestToken }) => storeRequestToken(requestToken))
...
const login = requestToken => {
  return fetchAccessToken(requestToken)
    .then(({ access_token }) => fetchSessionId(access_token))
    .then(({ session_id }) => storeLoginData(session_id))
    .then(() => displaySuccess('You have logged in!'));
};

const deleteSession = () => {
  return api.auth
    .delete_session(store.getState().auth.sessionId)
    .then(() => unstoreLoginData());
};
...
export { login, logout, createRequestToken, syncReduxWithLocalStorage };
